...
def is_point_in_rect(pt, rect):
    ...
    return (rx1 + margin <= x <= rx2 - margin) and (ry1 + margin <= y <= ry2 - margin)

def is_blocked(p1, p2, obstacles):
    """
    Checks if segment p1-p2 is blocked. 
    Uses 'Shrinking' logic to allow grazing corners (passing through tight gaps).
    """
    EPS = 1e-5 # Tolerance to allow grazing
    ...
    for obs in obstacles:
        ...
        if sx1 >= sx2 or sy1 >= sy2: continue


        if (sx1 <= p1[0] <= sx2 and sy1 <= p1[1] <= sy2) or \
           (sx1 <= p2[0] <= sx2 and sy1 <= p2[1] <= sy2):
            return True
        ...
        if intersect(p1, p2, c1, c2): return True # Bottom
        if intersect(p1, p2, c2, c3): return True # Right
        if intersect(p1, p2, c3, c4): return True # Top
        if intersect(p1, p2, c4, c1): return True # Left
...
def build_visibility_graph(obstacles, wafer_r):
    """
    Builds graph nodes including obstacle corners AND projected intersection points.
    Crucial for finding the 'center' of a cross or hole.
    """
    base_points = set()
    base_points.add((0.0, 0.0)) # Always try to add the center

    # 1. Add Corners
    for x1, y1, x2, y2 in obstacles:
        base_points.add((x1, y1))
        base_points.add((x2, y1))
        base_points.add((x2, y2))
        base_points.add((x1, y2))

    # 2. Add Projection Intersections (The "Armpits" of the shape)
    # If we have a cross or a hole, the relevant navigation points are defined
    # by the x-coordinates of vertical bars and y-coordinates of horizontal bars.
    xs = set()
    ys = set()
    for obs in obstacles:
        xs.add(obs[0]); xs.add(obs[2])
        ys.add(obs[1]); ys.add(obs[3])
    ...
    # 3. Create Safe Nodes around Base Points
    for bx, by in base_points:
        # Generate 4 candidates around each point of interest
        candidates = [
            (bx - margin, by - margin), 
            (bx + margin, by - margin),
            (bx + margin, by + margin), 
        ...
        for pt in candidates:
            if math.hypot(pt[0], pt[1]) > wafer_r: continue
    ...
    for node in nodes:
        is_dup = False
        for unode in unique_nodes:
            if math.hypot(node[0]-unode[0], node[1]-unode[1]) < 1e-4:
                is_dup = True
                break
        if not is_dup:
    ...
    nodes = unique_nodes
    print(f"Graph built with {len(nodes)} nodes.")
...
def build_matrix(dies, start_pos, start_angle, graph, nodes, obstacles, sv, sa, cv, ca):
    ...
    print("Calculating travel costs matrix...")
    for i in range(n):
